...
class LeaderBoardController extends GetxController implements GetxService {
  ...
  Future<void> getLeaderboardList(
    ...
    isLoading = true;
    Response? response = await leaderBoardServiceInterface.getLeaderboardList(
        offset, selectedFilterTypeName);
    if (response!.statusCode == 200) {
      if (offset == 1) {
        leaderBoardModel = LeaderBoardModel.fromJson(response.body);
      } else {
        leaderBoardModel!.data!
            .addAll(LeaderBoardModel.fromJson(response.body).data!);
        leaderBoardModel!.offset =
            LeaderBoardModel.fromJson(response.body).offset;
        leaderBoardModel!.totalSize =
            LeaderBoardModel.fromJson(response.body).totalSize;
      }
      isLoading = false;
    } else {
      isLoading = false;
