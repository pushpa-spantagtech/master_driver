...
class TripController extends GetxController implements GetxService {
  ...
  Future<Response> getTripList(int offset, String from, String to,
      String tripType, String filter) async {
    isLoading = true;
  ...
  //
  //   print("CANCEL API CALLING");
  //
  //   Response response = await tripServiceInterface
  //       .getTripOngoingAndAcceptedCancellationCauseList();
  //
  //   print("CANCEL API STATUS ==> ${response.statusCode}");
  //   print("CANCEL API RESPONSE ==> ${response.body}");
  //
  //   if(response.statusCode == 200){
  //
  //     tripCancellationCauseList =
  //         TripCancellationCauseList.fromJson(response.body);
  //
  //     print("CANCEL DATA STORED ==> $tripCancellationCauseList");
  //
  //   } else {
  //
  //     ApiChecker.checkApi(response);
