...
class TripRepository implements TripRepositoryInterface {
  ...
  Future<Response> getTripList(String tripType, String from, String to,
      int offset, String filter) async {
    return await apiClient.getData(
        '${AppConstants.tripList}?type=$tripType&limit=10&offset=$offset&filter=$filter');
  }
  ...
  Future<Response> getTripOverView(String filter) async {
    return await apiClient
        .getData('${AppConstants.tripOverView}?filter=$filter');
  }

  @override
  Future getTripOngoingAndAcceptedCancellationCauseList() async {
    return await apiClient
        .getData(AppConstants.getOngoingAndAcceptedCancellationCauseList);
  }
