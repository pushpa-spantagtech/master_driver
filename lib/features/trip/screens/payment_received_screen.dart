  ...
  void initState() {
    ...
    _paymentStatusTimer = Timer.periodic(
      ...
      (_) async {
        ...
        final Response response = await rideController.getRideDetails(
          tripId,
          fromHomeScreen: true,
        );
        ...
        body: SingleChildScrollView(
          child: GetBuilder<RideController>(
            builder: (finalFareController) {
              ...
              final finalFare = finalFareController.finalFare;

              final double vatTaxAmount = finalFare == null
                  ? 0
                  : (finalFare.vatTax ?? 0) > 0
                      ? (finalFare.vatTax ?? 0).toDouble()
                      : ((finalFare.paidFare ?? 0) -
                              (finalFare.actualFare ??
                                  finalFare.distanceWiseFare ??
                                  0) -
                              (finalFare.cancellationFee ?? 0) -
                              (finalFare.idleFee ?? 0) -
                              (finalFare.delayFee ?? 0) -
                              (finalFare.tips ?? 0) +
                              (finalFare.couponAmount ?? 0) +
                              (finalFare.discountAmount ?? 0))
                          .clamp(0, double.infinity)
                          .toDouble();

              return finalFare != null
                  ? Column(
