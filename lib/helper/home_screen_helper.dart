...
class HomeScreenHelper {
  void checkMaintanenceMode() {
    int ridingCount = (Get.find<RideController>().ongoingTrip == null ||
            Get.find<RideController>().ongoingTrip!.isEmpty)
        ? 0
        : (Get.find<RideController>().ongoingTrip![0].currentStatus ==
            ...
            : 0;
    int parcelCount =
        Get.find<RideController>().parcelListModel?.totalSize ?? 0;
  ...
  void pendingParcelListPusherImplementation() {
    for (int i = 0;
        i < Get.find<RideController>().getPendingRideRequestModel!.data!.length;
        i++) {
      PusherHelper().customerInitialTripCancel(
          Get.find<RideController>().getPendingRideRequestModel!.data![i].id!,
  ...
  void pendingLastRidePusherImplementation() {
    for (int i = 0;
        i < Get.find<RideController>().ongoingTripDetails!.length;
        i++) {
      List<String> data = ["ongoing", "accepted"];
      if (data.contains(
          Get.find<RideController>().ongoingTripDetails![i].currentStatus)) {
        PusherHelper().tripCancelAfterOngoing(
            Get.find<RideController>().ongoingTripDetails![i].id!);
        PusherHelper().tripPaymentSuccessful(
            Get.find<RideController>().ongoingTripDetails![i].id!);
        PusherHelper().customerCouponAppliedOrRemoved(
            Get.find<RideController>().ongoingTripDetails![i].id!);
      }
      if (Get.find<RideController>().ongoingTripDetails![i].currentStatus ==
              'completed' &&
          Get.find<RideController>().ongoingTripDetails![i].paymentStatus ==
              'unpaid') {
        PusherHelper().customerCouponAppliedOrRemoved(
            Get.find<RideController>().ongoingTripDetails![i].id!);
      }
